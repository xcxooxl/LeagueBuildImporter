...
namespace BuildImporter
{
    class LolApi
    {
        ...
        public LolApi()
        {
            ...
            Client.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Basic", Convert.ToBase64String(Encoding.ASCII.GetBytes(
                $"riot:{_token}")));
        ...
        public async Task Init()
        {
            ...
            this.Champions = GetSummonerChampions(this.SummonerId).Result;
            Items = this.GetGameItems().Result;

            parser = new GuideParser();
            parser.SetItems(this.Items);
            parser.SetChampions(this.Champions);
        }
        ...
        public async Task<Dictionary<string, Item>> GetGameItems()
        {
            ...
            var itemsJson = await client.GetStringAsync("https://ddragon.leagueoflegends.com/cdn/8.19.1/data/en_US/item.json");
        ...
        private async Task GetSummonerId()
        {
            ...
            this.SummonerId = summmoner.summonerId;
        }
        ...
        public async Task GetRunesData()
        {
            ...
            this.Categories = JsonConvert.DeserializeObject<RuneCategory[]>(runeCats);

            var perks = JsonConvert.DeserializeObject<RunePerk[]>(runePerks);
            this.Perks = perks.ToDictionary(x => x.name, x => x);
                        ...
                        foreach (ManagementObject mo in mos.Get())
                        {
                            if (mo["CommandLine"] != null)
                            {
                                ...
                                foreach (var attributes in CommandlineArray)
                                {
                                    ...
                                    {
                                        string[] ports = attributes.Split('=');
                                        _port = Int32.Parse(ports[1]);
                                    }
                                }
                                return;
                            }
